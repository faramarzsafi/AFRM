...
public class TestClass {
    ...
    public static void main(String[] args) {
        ...
        try {
            scanner = new Scanner(new java.io.File("test.txt"));
            int num_user = 1;
       ...
       //      Log.printLine("cost4 : "+ (double)Actions/(double)898);
             Log.printLine("COST : "+ (((((double)SLA_violation/(double)NUM) * 100*100))+((((double)SUM_Unused /(double)SUM_Unused_id )*5)) +  ((double)Actions/(double)998)));
    ...
    public static List<Host> createHostList(int hostsNumber) {
        ...
        hostList.add(new Host(1, new RamProvisionerSimple(ram), new BwProvisionerSimple(bw),
                KnowledgeManagement.rule(vmList.get(0).getSize(), hostList.get(0).getStorage()), peList, new VmSchedulerTimeShared(peList)));
        if (NumOfViolation.violation(vmList.get(0).getSize(), hostList.get(0).getStorage())) {
            SLA_violation++;
        }

        for (int i = 2; i < hostsNumber; i++) {
            storage = KnowledgeManagement.rule(vmList.get(i - 1).getSize(), hostList.get(i - 1).getStorage(), vmList.get(i - 2).getSize());
